...
metar_fetch <- function(con, start, end) {
  metar_query <- "SELECT * FROM f_metar_hourly($1, $2)"
  rs <- suppressWarnings(dbSendQuery(con, metar_query, list(start, end)))
  df_raw <- dbFetch(rs)
  dbClearResult(rs)

  df <- df_raw %>%
    mutate(
      timestamp = with_tz(observation_time, tzone = "US/Eastern"),
      timestamp = ceiling_date(timestamp, unit = "hour"),
      temp = temp_c * 9 / 5 + 32,                 # degC -> degC
      dewpt = dewpoint_c * 9 / 5 + 32,            # degC -> degC
      wspd = wind_speed_kt * 1.152,               # knots -> mph
      precip = precip_in,
      pressure = sea_level_pressure_mb * 0.02953  # mg -> in hg
    ) %>%
    select(timestamp, temp, dewpt, wspd, precip, pressure)
...
metar_predictors <- function (df_hour) {
  # hour gaps
  rle_hour <- rle(df_hour$missing_hour)
  rle_hour$lengths[rle_hour$values] %>% max # max = 11 hours
  ...
  # daily dataset
  df_day <- df_hour %>%
    index_by(date = as.Date(timestamp)) %>%
    summarise(
      n_day = n(),
      n_hour = sum(n_hour),
      temp_min = min(temp),
  ...
  # merge
  df_hour %>%
    select(timestamp, temp, dewpt, wspd, pressure) %>%
    mutate(date = as.Date(timestamp)) %>%
    left_join(df_p_hr, by = "timestamp") %>%
    left_join(df_p_day, by = "date") %>%
    left_join(
...
generate_predictors <- function(con, start, end) {
  ...
  df_weather <- metar_fetch(con, start, end)
  df_weather_predictors <- metar_predictors(df_weather)
